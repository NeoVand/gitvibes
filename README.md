...
- **Living fabric background** — a soft indigo pixel shader that breathes, dents under your cursor, and ripples through the page's glass surfaces (24 fps, honors reduced-motion)
- **23 hands-on exercises** with live success detection — a ✔ fires the moment the repo reaches the goal state, from the core loop to `bisect`, interactive rebase, hook guardrails, worktree fleets, and a three-messes-at-once capstone
